...
## New scope — PDF job report

| Item | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| Printable job report: summary page + one to-scale page per sheet | `geometry::pdf_export`, `commands::export_report`, frontend `EXPORT_FORMATS.pdf` | done | **New scope, no Electron counterpart.** Written entirely in Rust so nothing about it depends on a webview existing - the frontend is intended to become Rust later, and webview print-to-PDF would have to be thrown away. **Hand-rolled PDF writer, no crate**: `printpdf` resolves to a 30-plus-crate tree (font shaping, image codecs, a vector-graphics engine, an RNG) for a page that strokes polylines and writes Helvetica; what's actually needed is a subset of PDF 1.4 unchanged since 2001 (`m`/`l`/`h`/`S`, `BT`/`Tf`/`Td`/`Tj`/`ET`, one of the 14 standard fonts, no embedding) - about 200 lines. Consumes the same `SheetLayout`/`PlacedShape` `dxf_export`/`svg_export` take, straight from `build_export_layouts`, so the drawn page can't disagree with an exported DXF. **Every printed number is measured off the geometry being drawn** (`polygon_material_area` per layout) rather than from a parallel totals computation, for the same reason. `include_unplaced` is forced off - never-placed parts belong in the summary's shortfall line, not a fake extra sheet page. ponytail ceiling: built-in WinAnsi Helvetica means the report is **English-only** and non-ASCII layer names degrade to `?`; embedding a Unicode TTF is the upgrade path |

## New scope — manual result editing (drag, pin, re-nest the rest)

| Item | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| Drag a placed part, pin parts, repack around the pinned ones | `nesting::repack::repack_around_locked`, `nesting::placement::placement_is_valid`, `commands::validate_placement`, frontend `wireSheetEditing` | done | **New scope, no Electron counterpart.** Lock state lives on `PlacedPartDto` itself (`#[serde(default)] locked`), not a parallel list, so it can't drift from the geometry it describes and rides into `RepackSheetRequest` inside `placement.parts` with no new field. `repack_sheet` gains `locked: &[usize]`; an empty slice takes the untouched GA path, a non-empty one skips the GA entirely - with parts pinned there's no free permutation to evolve - and runs a greedy largest-first pass against them as `PlacedObstacle`s, the same pattern `consolidation` already uses. Refuses the whole repack (`None`) if any freed part can't be placed rather than dropping it. Honours per-part rotation rules, so a tidy-up can't re-rotate a grain-locked part. **Validity is decided in Rust**, by the same `has_material_overlap`/`has_material_outside_sheet` pair the engine accepts candidates with, against margin/spacing-padded geometry - the JS side only does a bounding-box green/red hint during the drag, and the drop can still be refused. Rejected alternatives: threading pre-placed obstacles through `place_parts`/`dispatch` (new parameter on the hot path), and baking locked parts into the sheet as `children` holes (~10 lines, but `inner_nfp.rs:65` requires `children.is_empty()` for the rectangle fast path, so every repack would drop to the general orbiting fallback) |

## New scope — per-part constraints (grain direction + mirror override)

| Item | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| Per-part allowed rotation set, per-part mirror allow/deny | `nesting::placement` (`PartRule`/`PartRules`/`rotation_steps`), `nesting::ga` (`random_angle`), `dto::expand_parts`, frontend ANGLES/FLIP columns | done | **New scope, no Electron counterpart.** Carried on `PlacementConfig`/`GaConfig` - the two structs already threaded through every engine entry point - so **no function signature in `nesting` changed**. Keyed by instance part id (masking `MIRROR_ID_BIT`), so neither `place_parts` nor `ga` needs `shape_ids` to look a rule up. The allowed set **replaces** the global grid rather than filtering it: filtering would make 180 unreachable at `rotations: 3`, and allowed angles need not lie on the grid at all. `rotation_steps` returns `(angle, delta)` pairs, not bare angles, because the callers rotate geometry incrementally while `advance_rotation` wraps at 360 - recomputing a delta from wrapped angles turns a `+90` into a `-270`. An unconstrained part gets exactly the sequence the old fixed loop produced, deltas included, and an unconstrained GA run draws the identical random sequence (one `gen_range`, same slot count) - both are asserted by tests. A part's *effective* mirror flag decides whether `expand_parts` registers its flipped variant at all, so a mirror-denied part has no mirrored geometry for a stray gene to reach. `RunNestResponse::part_rules` is authoritative and rides back into `RepackSheetRequest`, so a manual repack can't re-rotate a grain-locked part |
| `BestResultDto` gains `config` (and `part_rules`) | `src-tauri` | done | Pre-existing bug, fixed here because per-part rules made it worse: a recovered result set `lastNestRequest` with no `.config`, so REPACK on it threw. Both fields are `#[serde(default)]`, so an older `best_result.json` still loads - the UI reports that repack is unavailable rather than failing at click time |
...
| Item | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| Profiles drawn as loose `LINE`/partial-`ARC`/open-polyline networks | `geometry::dxf_import` (`chain_edges`, `entities_to_polygons_chained`) | done | The per-entity pass (`entity_to_polygon`) has no cross-entity state by design, so chaining is a **second** pass over what it rejected, not another match arm. Greedy end-to-end walk; a chain that runs out of neighbours without closing is **discarded** rather than closed with an invented segment. Never joins across layers (cut/etch/drill are different operations). Join radius is `curve_tolerance`, not `polygon::TOL` - 1e-9 is float noise, real CAD endpoints that are meant to coincide miss by far more. Chained rings carry no `real_boundary`, so their arcs export tessellated |
| The older `POLYLINE`/`VERTEX` entity | `geometry::dxf_import` (`polyline_real_vertices`, `polyline_profile`) | done | Nearly free: the `dxf` crate re-assembles `VERTEX`/`SEQEND` into the entity before we see it, so `POLYLINE` and `LWPOLYLINE` were unified onto one `RealVertex`-based path rather than given two near-copies. 3D polylines / polyface meshes rejected outright, not flattened |
| `INSERT` block references, incl. nested + `MINSERT` arrays | `geometry::dxf_import::expand_inserts` | done | Needs the whole `Drawing` (block bodies aren't in `entities()`). Implements DXF's layer-`0` inheritance rule explicitly - block geometry on layer `0` takes the INSERT's layer. Recursion capped at `MAX_INSERT_DEPTH = 8` so a cyclic block reference terminates instead of hanging. A non-uniform INSERT scale would turn a circle into an ellipse, which no DXF circle/arc entity can express, so those are tessellated into an LWPOLYLINE instead of emitting a wrong circle. Local 2x3 affine, deliberately not a revived general `matrix.ts` |
| Fixture | `tests/fixtures/line-network.dxf` | done | Generated reproducibly by `crates/geometry/examples/gen_line_network.rs`. Imports as 1 shape without chaining, 3 (nesting to 1 part + 2 holes) with it |

## New scope — mirrored (flipped-over) parts

| Item | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| Let the search also try each part flipped over | `geometry::dxf_import::mirror_layered_polygon`, `nesting::ga` (`GaConfig::mirror`), `nesting::dispatch` (`MIRROR_ID_BIT`/`decode_rotation`), `dto::expand_parts`, frontend `cfg-mirror` | done | **New scope, no Electron counterpart** (the original app has no mirroring at all). Carried as an *extra part variant*, not a new gene: `expand_parts` registers each copy's mirror under `id ^ MIRROR_ID_BIT` with its own source id (its own NFP cache entries - a mirror's NFP differs from the original's), and the GA's rotation gene grid is drawn twice as long, where `>= 360` means "this angle, mirrored" (`ga::random_angle`). `dispatch::run_generation` is the only decoder. Nothing else changed: placement, consolidation, repack, export and the frontend all see a mirrored part as just another part id with its own geometry in `parts_by_id`. A gene whose variant isn't registered (a single-sheet repack passes only that sheet's parts) degrades to the un-flipped part instead of panicking. With `runs > 1`, run 1 is always executed *un*-mirrored (`escalated_run_config`) so the escalation always has a flip-free baseline for `is_better_nest` to pick against - mirroring only widens the search space (every un-flipped arrangement is still reachable in a mirrored run), but a wider space on the same budget can land worse. UI: off by default, deliberately loud while on (row + RUN button both turn `--error`), and **never restored from the saved config** - mirroring material that has a good side produces scrap, so it has to be ticked on purpose every session |
